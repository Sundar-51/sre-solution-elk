...
master3=$(aws ec2 describe-instances --query "Reservations[0].Instances[1].PrivateIpAddress" --filters "Name=tag-value,Values=ElasticSearch-Master" "Name=instance-state-name,Values=running" --region $region --output=text);
master4=$(aws ec2 describe-instances --query "Reservations[1].Instances[1].PrivateIpAddress" --filters "Name=tag-value,Values=ElasticSearch-Master" "Name=instance-state-name,Values=running" --region $region --output=text);
...
sed -i "s|#node.name: node-1|node.name: Els-slave|g" /etc/elasticsearch/elasticsearch.yml
...
sed -i '$ a node.master: false' /etc/elasticsearch/elasticsearch.yml
if [ $count == 2 ]
then
    sed -i 's|"host1", "host2"|"'"$master1"'", "'"$master2"'", "'"$hostname"'"|g' /etc/elasticsearch/elasticsearch.yml
	sed -i 's|"node-1", "node-2"|"'"$master1"'", "'"$master2"'"|g' /etc/elasticsearch/elasticsearch.yml
elif [ $count==3 ]
then
	sed -i 's|"host1", "host2"|"'"$master1"'", "'"$master2"'", "'"$master3"'", "'"$hostname"'"|g' /etc/elasticsearch/elasticsearch.yml
	sed -i 's|"node-1", "node-2"|"'"$master1"'", "'"$master2"'", "'"$master3"'"|g' /etc/elasticsearch/elasticsearch.yml
else 
    sed -i 's|"host1", "host2"|"'"$master1"'", "'"$master2"'", "'"$master3"'", "'"$master4"'", "'"$hostname"'"|g' /etc/elasticsearch/elasticsearch.yml
	sed -i 's|"node-1", "node-2"|"'"$master1"'", "'"$master2"'", "'"$master3"'", "'"$master4"'"|g' /etc/elasticsearch/elasticsearch.yml
fi