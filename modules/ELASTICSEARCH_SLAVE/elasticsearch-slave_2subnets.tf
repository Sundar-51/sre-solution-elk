...
resource "aws_launch_configuration" "els_slave" {
  name          = "els-slave"
  ...
  #user_data= file("./modules/ELASTICSEARCH_SLAVE/elasticsearch_slave_2subnets.sh")
  user_data = <<EOF
  #!/bin/bash
  ...
  sudo wget https://${var.aws_s3_shell_bucket_name}.s3.${var.region}.amazonaws.com/elasticsearch_slave.sh
  sudo chmod 777 elasticsearch_slave.sh
  sudo cp elasticsearch_slave.sh /etc/elasticsearch
  sudo sh /etc/elasticsearch/elasticsearch_slave.sh
  sudo systemctl restart elasticsearch
  EOF
  count = var.private_subnets == 2 ? 1 : 0
  }
#AUTO SCALING GROUP INSTANCES
resource "aws_autoscaling_group" "ELSlave" {
  desired_capacity   = var.no_of_elasticsearch_slave_nodes
  max_size           = 5
  min_size           = 1
  vpc_zone_identifier       = var.private_subnets
  launch_configuration      = aws_launch_configuration.els_slave[0].name
  tag {
    key = "Name"
    value = var.els_slave
